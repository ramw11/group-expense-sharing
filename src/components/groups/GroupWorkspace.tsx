    ...
    <div className="page-shell workspace">
      ...
      <div className="workspace-columns">
        <section className="panel workspace-panel">
          <div className="panel-title-row"><div><p className="section-kicker">{t("whoPays")}</p><h2>{t("billingUnits")}</h2></div></div>
          <div className="inline-form unit-form"><label className="sr-only" htmlFor="unit-name">{t("billingUnits")}</label><input id="unit-name" value={name} onChange={(event) => setName(event.target.value)} onKeyDown={(event) => event.key === "Enter" && submitUnit()} placeholder={t("unitPlaceholder")} /><button className="primary-button" onClick={submitUnit}><Plus size={19} /> {editingId ? t("save") : t("add")}</button></div>
          <div className="unit-list">
            {sortedUnits.length === 0 ? <div className="small-empty">Add the first person or household that pays together.</div> : sortedUnits.map((unit, index) => {
              ...
              return <article className={`unit-row ${effectiveUnitId === unit.id ? "selected" : ""}`} key={unit.id}>
                ...
                <button className="unit-name" onClick={() => { setSelectedUnitId(unit.id); setEditingMemberId(null); setMemberForm(initialMemberForm); }}><strong>{unit.name}</strong><span>{memberCount} {memberCount === 1 ? t("member") : t("members")}</span></button>
        ...
        <section className="panel member-panel">
          <div className="member-panel-head"><div><p className="section-kicker">{t("people")}</p><h2>{selectedUnit?.name ?? t("selectUnit")}</h2></div><div className="member-count">{unitMembers.length}</div></div>
          {selectedUnit && <>
