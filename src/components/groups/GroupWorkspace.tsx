...
import type { BillingUnit, GatheringDraft, Group, Language, Member } from "../../domain/models";
        ...
        <div className="unit-list">
          {sortedUnits.length === 0 ? <div className="small-empty">{t("noFamilies")}</div> : sortedUnits.map((unit, index) => {
            ...
            return <article className={`unit-row ${effectiveUnitId === unit.id ? "selected" : ""}`} key={unit.id}>
              ...
              <button className="unit-name" onClick={() => { setSelectedUnitId(unit.id); setEditingMemberId(null); setMemberForm(initialMemberForm); }}><strong>{unit.name}</strong><span>{memberCount} {memberCount === 1 ? t("member") : t("members")} · {linkedCount} {t("eventsLabel")}</span></button>
