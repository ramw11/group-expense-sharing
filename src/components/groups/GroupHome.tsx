import { ArrowUpRight, Cloud, CloudAlert, Link2, LoaderCircle, Pencil, Plus, Settings, SlidersHorizontal, Trash2, UsersRound } from "lucide-react";
...
interface GroupHomeProps {
  groups: Group[]; drafts: GatheringDraft[]; sharedGroups: SharedGroupConnection[]; cloudStatus: "idle" | "syncing" | "synced" | "error"; cloudMessage: string; language: Language; onLanguageChange(language: Language): void;
  onCreate(name: string): void; onRename(id: string, name: string): void; onDelete(id: string): void;
  onStart(id: string): void; onManage(id: string): void; onShare(id: string): void; onSettings(): void;
}

export function GroupHome({ groups, drafts, sharedGroups, cloudStatus, cloudMessage, language, onLanguageChange, onCreate, onRename, onDelete, onStart, onManage, onShare, onSettings }: GroupHomeProps) {
  ...
  return <div className="page-shell">
    ...
    <section className="group-grid" aria-label={t("circles")}>
      {groups.length === 0 ? <div className="empty-card"><div className="empty-icon"><UsersRound size={28} /></div><h3>{t("firstGroup")}</h3><p>{t("firstGroupCopy")}</p></div> : groups.map((group, index) => { const draft = drafts.find((item) => item.groupId === group.id); const hasDraft = Boolean(draft); const isShared = sharedGroups.some((item) => item.groupId === group.id); return <article className={`group-card tone-${index % 3}`} key={group.id}>
        ...
        <div className="group-choices"><button className="event-button" onClick={() => onStart(group.id)}>{hasDraft ? t("continueEvent") : t("startEvent")} <ArrowUpRight size={18} /></button><button className="manage-button" onClick={() => onManage(group.id)}><SlidersHorizontal size={17} /> {t("manageGroup")}</button><button className={`share-button ${isShared ? "shared" : ""}`} onClick={() => onShare(group.id)}>{isShared ? <Cloud size={17} /> : <Link2 size={17} />} {isShared ? t("sharedGroup") : t("shareGroup")}</button></div>
      </article>; })}
    </section>
  </div>;
}
