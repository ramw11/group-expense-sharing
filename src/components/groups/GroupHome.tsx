...
import type { Group, Language } from "../../domain/models";
import { translate } from "../../i18n";
import { IconButton } from "../ui/IconButton";
import { LanguageToggle } from "../ui/LanguageToggle";

interface GroupHomeProps {
  groups: Group[]; language: Language; onLanguageChange(language: Language): void;
...
export function GroupHome({ groups, language, onLanguageChange, onCreate, onRename, onDelete, onStart, onManage, onSettings }: GroupHomeProps) {
  ...
  return <div className="page-shell">
    ...
    <section className="group-grid" aria-label={t("circles")}>
      {groups.length === 0 ? <div className="empty-card"><div className="empty-icon"><UsersRound size={28} /></div><h3>{t("firstGroup")}</h3><p>{t("firstGroupCopy")}</p></div> : groups.map((group, index) => <article className={`group-card tone-${index % 3}`} key={group.id}>
        ...
        <button className="group-open" onClick={() => onStart(group.id)}><span>{group.name}</span><ArrowUpRight size={28} /></button>
        <div className="group-choices"><button className="event-button" onClick={() => onStart(group.id)}>{t("startEvent")} <ArrowUpRight size={18} /></button><button className="manage-button" onClick={() => onManage(group.id)}><SlidersHorizontal size={17} /> {t("manageGroup")}</button></div>
      </article>)}
    </section>
  </div>;
}
