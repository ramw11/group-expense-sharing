...
interface GroupHomeProps {
  groups: Group[]; draftGroupIds: string[]; language: Language; onLanguageChange(language: Language): void;
...
export function GroupHome({ groups, draftGroupIds, language, onLanguageChange, onCreate, onRename, onDelete, onStart, onManage, onSettings }: GroupHomeProps) {
  ...
  return <div className="page-shell">
    ...
    <section className="group-grid" aria-label={t("circles")}>
      {groups.length === 0 ? <div className="empty-card"><div className="empty-icon"><UsersRound size={28} /></div><h3>{t("firstGroup")}</h3><p>{t("firstGroupCopy")}</p></div> : groups.map((group, index) => { const hasDraft = draftGroupIds.includes(group.id); return <article className={`group-card tone-${index % 3}`} key={group.id}>
        ...
        <button className="group-open" onClick={() => onStart(group.id)}><span>{group.name}{hasDraft && <small>{t("draftLabel")}</small>}</span><ArrowUpRight size={28} /></button>
