...
export function EventScreen({ group, repositoryFamilies, repositoryMembers, settings, language, draft, cloudStatus, cloudMessage, onLanguageChange, onSave, onShare, onCreateFamily, onBack, onEditGroup }: EventScreenProps) {
  ...
  const [expenseReporterId, setExpenseReporterId] = useState(activeMembers[0]?.id ?? "");
  ...
  const addExpense = () => {
    ...
    const reporter = activeMembers.find((member) => member.id === expenseReporterId);
    if (!reporter || !Number.isFinite(amount) || amount <= 0) return;
    setExpenses((current) => [...current, { id: createId(), billingUnitId: reporter.billingUnitId, reportedByMemberId: reporter.id, amount, description: expenseDescription.trim() || undefined, notes: expenseNotes.trim() || undefined, receiptUrl }]);
    setExpenseUnitId(reporter.billingUnitId);
    setExpenseAmount(""); setExpenseDescription(""); setExpenseNotes(""); setReceiptUrl(undefined); setScanState({ status: "idle", progress: 0 });
      ...
      <main className="gathering-main">
        ...
        <section className="expense-section">
          ...
          <div className={`expense-grid ${expenseFormOpen ? "" : "form-closed"}`}>
            {expenseFormOpen && <div className="expense-entry">
              <label><span>{t("reportedBy")}</span><select value={expenseReporterId} onChange={(event) => { const reporter = activeMembers.find((member) => member.id === event.target.value); setExpenseReporterId(event.target.value); setExpenseUnitId(reporter?.billingUnitId ?? ""); }}><option value="">{language === "he" ? "בחירת מדווח" : "Choose reporter"}</option>{activeMembers.map((member) => <option value={member.id} key={member.id}>{member.name} — {unitName(member.billingUnitId)}</option>)}</select></label>
              <label><span>{t("paidBy")}</span><input value={unitName(expenseUnitId)} readOnly /></label>
              ...
              <label className="expense-description"><span>{t("expenseNotes")} <i>{t("optional")}</i></span><textarea value={expenseNotes} onChange={(event) => setExpenseNotes(event.target.value)} /></label>
        ...
        <section className="settlement-section">
          ...
          {calculation.totalWeight === 0 ? <div className="calculation-empty">{t("chooseAttendee")}</div> : calculation.totalPaid === 0 ? <div className="calculation-empty">{t("addExpensePrompt")}</div> : <>
            ...
            {dashboardOpen && <ExpenseDashboard units={units} members={repositoryMembers} expenses={expenses} calculation={calculation} currency={settings.currency} language={language} eventName={eventName.trim() || group.name} eventDate={date} settlements={settlements} />}
