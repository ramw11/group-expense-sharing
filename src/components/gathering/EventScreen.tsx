      ...
      <main className="gathering-main">
        ...
        <section className="expense-section">
          ...
          <div className={`expense-grid ${expenseFormOpen ? "" : "form-closed"}`}>
            ...
            {expenses.length > 0 && <button className={`expense-amount-save ${saved ? "saved" : ""}`} disabled={cloudStatus === "syncing"} onClick={save}>{cloudStatus === "syncing" ? <LoaderCircle className="spin" size={18} /> : saved ? <Check size={18} /> : <Save size={18} />} {saved ? t("draftSaved") : t("saveAmounts")}</button>}
          </div>
        </section>
